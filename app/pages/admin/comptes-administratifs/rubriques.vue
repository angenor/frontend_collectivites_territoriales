  ...
  <div>
    ...
    <div class="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
      <div>
        ...
        <input v-model="searchQuery" type="text" placeholder="Code ou intitulé..." class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
      </div>
      <div>
        ...
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-700/50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Intitulé</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="rubrique in paginatedRubriques" :key="rubrique.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
              ...
              <td class="px-6 py-4 text-sm">
                <span v-if="rubrique.section" :class="rubrique.section === 'fonctionnement' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400' : 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400'" class="px-2 py-1 rounded-full text-xs font-medium capitalize">
                  {{ rubrique.section }}
                </span>
                <span v-else class="text-gray-400">-</span>
              </td>
              <td class="px-6 py-4 text-sm">
                <span :class="rubrique.type === 'recette' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : rubrique.type === 'depense' ? 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300'" class="px-2 py-1 rounded-full text-xs font-medium capitalize">
                  {{ rubrique.type }}
                </span>
              </td>
              ...
              <td class="px-6 py-4 text-sm">
                <button @click="toggleRubrique(rubrique)" :class="rubrique.est_active ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300'" class="px-2 py-1 rounded-full text-xs font-medium cursor-pointer">
                  {{ rubrique.est_active ? 'Actif' : 'Inactif' }}
                </button>
              </td>
              <td class="px-6 py-4 text-right text-sm">
                <button @click="editRubrique(rubrique)" class="cursor-pointer text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 mr-3">
                  Modifier
                </button>
                <button @click="deleteRubrique(rubrique.id)" class="cursor-pointer text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300">
                  Supprimer
                </button>
              </td>
            </tr>
...
const filteredRubriques = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    result = result.filter(r =>
      ...
      r.intitule.toLowerCase().includes(query)
    )
  }
  ...
  if (filterActif.value !== '') {
    ...
    result = result.filter(r => r.est_active === isActif)
  }
...
const rubriquesByType = computed(() => ({
  recette: rubriques.value.filter(r => r.type === 'recette').length,
  depense: rubriques.value.filter(r => r.type === 'depense').length
}))

const rubriquesByStatus = computed(() => ({
  actif: rubriques.value.filter(r => r.est_active).length,
  inactif: rubriques.value.filter(r => !r.est_active).length
}))
...
async function toggleRubrique(rubrique: RubriqueBudgetaire) {
  await toggleRubriqueActive(rubrique.id, !rubrique.est_active)
  await fetchRubriques()
}
